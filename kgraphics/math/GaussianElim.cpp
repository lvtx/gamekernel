...
Solve( float* b, float* A, unsigned int n )
{
    ...
    for ( unsigned int pivot = 0; pivot < n; ++pivot )
    {
        ...
        float maxelem = gfx::Abs( A[ maxrow + n*pivot ] );
        for ( row = pivot+1; row < n; ++row )
        {
            float elem = gfx::Abs( A[ row + n*pivot ] );
            if ( elem > maxelem )
            {
                maxelem = elem;
                maxrow = row;
...
Determinant( float* A, unsigned int n )
{
    ...
    for ( unsigned int pivot = 0; pivot < n; ++pivot )
    {
        ...
        // find the largest magnitude element in the current column
        unsigned int maxrow = pivot;
        float maxelem = gfx::Abs( copy[ maxrow + n*pivot ] );
        for ( row = pivot+1; row < n; ++row )
        {
            float elem = gfx::Abs( copy[ row + n*pivot ] );
            if ( elem > maxelem )
            {
                maxelem = elem;
                maxrow = row;
