...
#include <kcore/util/Crc32.h>

static const UINT CRC32_INITIAL_HASH = 0xFFFFFFFF;

static const UINT s_crc32table[256] = 
{
...
uint
Crc32::Calculate( const byte* src, uint len )
{
    uint hash = CRC32_INITIAL_HASH;

    for(; len> 0; --len)
    {
        hash = ( ( ( hash >> 8 ) & 0x00FFFFFF )^
            s_crc32table[ ( hash^ *src++ ) & 0x000000FF ] );
    }
